...
<div class="isi">
    Yang bertanda tangan dibawah ini Kepala Desa Ciasmara Kecamatan Pamijahan Kabupaten Bogor, menerangkan bahwa :<br><br>

    Nama: {{ $surat->nama }}<br>
    Tempat/Tgl Lahir: {{ $surat->tempat_lahir }}, {{ \Carbon\Carbon::parse($surat->tanggal_lahir)->translatedFormat('d-m-Y') }}<br>
    Pekerjaan: {{ $surat->pekerjaan }}<br>
    Agama: {{ $surat->agama }}<br>
    Alamat: {{ $surat->alamat }}<br><br>

    Telah meninggal dunia pada:<br>
    Hari: {{ $surat->hari_kematian }}<br>
    Tanggal: {{ $surat->tanggal_kematian }}<br>
    Pukul: {{ $surat->waktu_kematian }}<br>
    Di: {{ $surat->tempat_kematian }}<br>
    Disebabkan: {{ $surat->sebab_kematian }}<br><br>
