...
<body>
    @php
        ...
        $letterNumber = '000.1.7.3/' . $letterRequest->request_number . '/' . $romanMonth . '/' . $currentYear;
        
        $barangHilang = $formData['barang_hilang'] ?? '-';
        $tempatHilang = $formData['tempat_hilang'] ?? '-';
        $tanggalHilang = isset($formData['tanggal_hilang']) ? \Carbon\Carbon::parse($formData['tanggal_hilang'])->locale('id')->translatedFormat('d F Y') : '-';
    ...
    <div class="content">
        <p style="text-indent: 50px; text-align: justify;">Yang bertanda tangan dibawah ini, Kepala Desa Ciasmara Kecamatan Pamijahan Kabupaten Bogor dengan ini menerangkan bahwa:</p>
        ...
        <p style="text-align: justify;">Nama tersebut diatas telah kehilangan <strong>{{ $barangHilang }}</strong> pada tanggal <strong>{{ $tanggalHilang }}</strong> di <strong>{{ $tempatHilang }}</strong>.</p>
        
        <p></p>
        <p style="text-indent: 50px; text-align: justify;">Demikian surat keterangan ini dibuat berdasarkan laporan yang bersangkutan, untuk pihak terkait agar dapat mengetahui dan sebagai bahan lebih lanjut.</p>
    </div>
