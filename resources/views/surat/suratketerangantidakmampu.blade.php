...
<body>
    @php
        ...
        $letterNumber = '400.9.3.3/' . $letterRequest->request_number . '/' . $romanMonth . '/' . $currentYear;
        
        $keperluan = $formData['keperluan'] ?? 'bantuan sosial';
        $penghasilanPerbulan = $formData['penghasilan_perbulan'] ?? '0';
        $jumlahTanggungan = $formData['jumlah_tanggungan'] ?? '0';
        $kondisiRumah = $formData['kondisi_rumah'] ?? 'sederhana';
        $pekerjaanUtama = $formData['pekerjaan_utama'] ?? $job;
    @endphp
    ...
    <div class="content">
        ...
        <div class="data-table">
            <table>
                ...
                <tr><td>Pekerjaan</td><td>:</td><td><strong>{{ $pekerjaanUtama }}</strong></td></tr>
                <tr><td>Alamat</td><td>:</td><td><strong>{{ $fullAddress }}</strong></td></tr>
                <tr><td>Penghasilan/Bulan</td><td>:</td><td><strong>Rp {{ number_format($penghasilanPerbulan, 0, ',', '.') }}</strong></td></tr>
                <tr><td>Jumlah Tanggungan</td><td>:</td><td><strong>{{ $jumlahTanggungan }} Orang</strong></td></tr>
                <tr><td>Kondisi Rumah</td><td>:</td><td><strong>{{ $kondisiRumah }}</strong></td></tr>
            </table>
        </div>

        <p>Orang tersebut diatas adalah benar warga Desa Ciasmara yang tergolong keluarga kurang mampu dan surat keterangan ini dipergunakan untuk <strong>{{ $keperluan }}</strong>.</p>
